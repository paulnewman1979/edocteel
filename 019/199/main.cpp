...
public:
    vector<int> rightSideView(TreeNode* root) {
        ...
        if (NULL != root) {
            ...
            int level = 1;
            checkRightView(root->left, level, results);
            checkRightView(root->right, level, results);
        }
...
private:
    void checkRightView(TreeNode* root, int level, vector<int>& results) {
        if (NULL == root) {
            return;
        }
        ...
        checkRightView(root->left, level + 1, results);
        checkRightView(root->right, level + 1, results);
    }
};
